...
void projectLidarToCamera2()
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        // 4. Add Filter to remove unnecessary points
        double maxX = 25.0, maxY = 6.0, minZ = -1.4, minR = 0.01;
        if (it->x > maxX || it->x < 0.0 || abs(it->y) > maxY || it->z < minZ || it->r < minR){
            continue;
        }

        // 1. Convert current Lidar point into homogeneous coordinates and store it in the 4D variable X.
        //X.push_back({(*it).x, (*it).y, (*it).z, 1.0});
